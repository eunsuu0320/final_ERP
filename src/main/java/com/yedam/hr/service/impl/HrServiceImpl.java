...
public class HrServiceImpl implements HrService {
	...
	public void updateEmployee(Employee employee, MultipartFile signImg, MultipartFile pdfFile) {
	    ...
	    if (employee.getDept() != null) existing.setDept(employee.getDept());
	    ...
	    if (employee.getResignReason() != null) existing.setResignReason(employee.getResignReason());
	    if (employee.getBankCode() != null) existing.setBankCode(employee.getBankCode());
	    if (employee.getAccHolder() != null) existing.setAccHolder(employee.getAccHolder());
	    if (employee.getAccNo() != null) existing.setAccNo(employee.getAccNo());
	 ...
	 // 4. 선택적으로 서명 이미지 저장 (있으면 갱신)
	    HrSign sign = hrSignRepository.findByEmpNo(existing.getEmpNo())
	            .orElse(null);
