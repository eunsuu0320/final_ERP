...
function diffToWorkTime(on, off) {
	if (!on || !off) return "";
	const ms = new Date(off) - new Date(on);
	...
	commuteTable = new Tabulator(document.getElementById("commute-table"), {
		...
		columns: [
			...
			},
			{
				...
				field: "workTime",
				hozAlign: "center",
				mutator: (value, data) => diffToWorkTime(data.onTime, data.offTime),
			},
			{ title: "비고", field: "note" },
		],
	});
...
async function loadCommutes() {
	...
	const rows = (data || []).map(r => ({
		...r,
		workTime: diffToWorkTime(r.onTime, r.offTime),
	}));

	// ✅ 올바른 테이블 인스턴스에 주입
	commuteTable.setData(rows);
}
...
if (printBtn) {
	printBtn.addEventListener("click", () => {
      ...
      <tr>
        ...
        <td>${diffToWorkTime(r.onTime, r.offTime)}</td>
        <td>${r.note ?? ""}</td>
      </tr>
    `).join("");
...
document.getElementById("onoff-excel").addEventListener("click", async () => {
	...
	const exportRows = rows.map(r => ({
		...
		"근무시간": diffToWorkTime(r.onTime, r.offTime),
		"비고": r.note ?? ""
	}));
