...
// 0시간 00분이면 빈 문자열 반환 (서버 Double(시간) → 표시용 텍스트)
function fmtHourFromDouble(hoursVal) {
  if (hoursVal == null || isNaN(hoursVal)) return null; // 서버값 없음 → null
  const totalMin = Math.round(Number(hoursVal) * 60);
  if (totalMin <= 0) return ""; // 0이면 빈칸
...
// ===== Excel(선택건만) =====
function getCommuteTable() {
  if (window.commuteTable && typeof window.commuteTable.getSelectedData === "function") {
    return window.commuteTable;
  }
  if (window.Tabulator) {
    const found = Tabulator.findTable("#commute-table");
...
function mapRowsForExport(rows) {
  return rows.map(r => {
    ...
    return {
      ...
      "사원": (typeof EMP_MAP !== "undefined" ? (EMP_MAP[r.empCode] || r.empCode) : r.empCode) || "",
...
document.getElementById("onoff-excel").addEventListener("click", async () => {
  const table = getCommuteTable();
  if (!table) {
    alert("테이블이 아직 준비되지 않았습니다. (#commute-table / 전역 commuteTable 확인)");
    console.warn("Tabulator instance not found.");
    return;
  }
  ...
  try {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "전체 출퇴근");
    XLSX.writeFile(wb, "commute_all.xlsx");
  } catch (e) {
    console.error(e);
